...
class EventEmbedding(nn.Module):
    def __init__(self, vocab_size: int, emb_dim: int = 64, dropout: float = 0.20):
        ...
        self.time_proj = nn.Sequential(nn.Linear(2, emb_dim), nn.GELU(), nn.Dropout(dropout), nn.Linear(emb_dim, emb_dim))
        self.norm = nn.LayerNorm(emb_dim)
        self.dropout = nn.Dropout(dropout)

    def forward(self, tokens, time_features):
        return self.dropout(self.norm(self.token_emb(tokens) + self.time_proj(time_features)))
...
class RNNClassifier(nn.Module):
    def __init__(self, vocab_size: int, rnn_type: str = "GRU", emb_dim: int = 64, hidden_dim: int = 128, num_layers: int = 1, dropout: float = 0.20):
        super().__init__()
        self.event_emb = EventEmbedding(vocab_size, emb_dim, dropout)
        rnn_cls = nn.GRU if rnn_type.upper() == "GRU" else nn.LSTM
        self.rnn = rnn_cls(emb_dim, hidden_dim, num_layers=num_layers, batch_first=True, dropout=0 if num_layers == 1 else dropout)
...
class RetainLiteClassifier(nn.Module):
    def __init__(self, vocab_size: int, emb_dim: int = 64, hidden_dim: int = 128, dropout: float = 0.20):
        ...
        self.head = nn.Sequential(nn.Dropout(dropout), nn.Linear(emb_dim, 1))
...
def make_model(model_name: str, vocab_size: int) -> nn.Module:
    if model_name == "GRU_1L":
        return RNNClassifier(vocab_size, rnn_type="GRU", num_layers=1)
    if model_name == "GRU_2L":
        return RNNClassifier(vocab_size, rnn_type="GRU", num_layers=2)
    if model_name == "LSTM_1L":
        return RNNClassifier(vocab_size, rnn_type="LSTM", num_layers=1)
    if model_name == "LSTM_2L":
        return RNNClassifier(vocab_size, rnn_type="LSTM", num_layers=2)
    if model_name == "RETAIN_lite":
        return RetainLiteClassifier(vocab_size)
...
def load_vocab(data_dir: Path, task: str, version: str) -> dict:
    path = data_dir / task / version / "vocab.json"
...
def make_loaders(df: pd.DataFrame, batch_size: int, max_len: int, seed: int, num_workers: int):
    split_dfs = {split: df[df["split"] == split].copy().reset_index(drop=True) for split in ["train", "tuning", "held_out"]}
    generator = torch.Generator()
    generator.manual_seed(seed)
    loaders = {
        "train": DataLoader(EHRSequenceDataset(split_dfs["train"], max_len), batch_size=batch_size, shuffle=True, generator=generator, num_workers=num_workers, collate_fn=collate_sequence_batch),
        "tuning": DataLoader(EHRSequenceDataset(split_dfs["tuning"], max_len), batch_size=batch_size, shuffle=False, num_workers=num_workers, collate_fn=collate_sequence_batch),
        "held_out": DataLoader(EHRSequenceDataset(split_dfs["held_out"], max_len), batch_size=batch_size, shuffle=False, num_workers=num_workers, collate_fn=collate_sequence_batch),
    }
    return loaders
...
def run_inference(model, loader, device):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            logits = model(b["tokens"], b["time_features"], b["mask"], b["lengths"])
...
def train_model(model, loaders, args, device):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        p_tuning = sigmoid_np(tuning["logits"])
        m = binary_ranking_metrics(tuning["y_true"], p_tuning)
...
def evaluate_with_platt(task, version, model_name, seed, model, loaders, device):
    ...
    try:
        ...
        calibrator.fit(tuning["logits"].reshape(-1, 1), tuning["y_true"].astype(int))
    ...
    for calibration, risk in probs.items():
        ...
        result_rows.append({"task": task, "family": "sequence", "source": "sequence", "version": version, "model": model_name, "seed": seed, "calibration": calibration, **{f"heldout_{k}": v for k, v in m.items()}})
        pred_frames.append(pd.DataFrame({"task": task, "family": "sequence", "source": "sequence", "version": version, "model": model_name, "seed": seed, "calibration": calibration, "row_id": heldout["row_id"].astype(int), "subject_id": heldout["subject_id"].astype(int), "y_true": heldout["y_true"].astype(int), "logit": heldout["logits"], "risk": risk}))
...
def run_one(args, cfg: dict, seed: int, device):
    ...
    loaders = make_loaders(df, args.batch_size, args.max_len, seed, args.num_workers)
    model = make_model(cfg["model"], vocab_size=len(vocab))
    ...
    if args.save_checkpoints:
        ...
        torch.save({"task": cfg["task"], "version": cfg["version"], "model": cfg["model"], "seed": seed, "vocab_size": len(vocab), "max_len": args.max_len, "state_dict": model.state_dict()}, ckpt_dir / f"{stem}.pt")
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("ehrshot_multiseed_sequence_results"))
    ...
    parser.add_argument("--clearml-task-name", type=str, default="sequence_multiseed_stability")
    ...
    for cfg in DEFAULT_CONFIGS:
        for seed in seeds:
            print("=" * 100)
            print(f"Sequence experiment: {cfg} seed={seed}")
    ...
    results_df.to_csv(args.output_dir / "sequence_multiseed_results.csv", index=False)
    predictions_df.to_csv(args.output_dir / "sequence_multiseed_heldout_predictions.csv", index=False)
    topk_df.to_csv(args.output_dir / "sequence_multiseed_topk.csv", index=False)
    history_df.to_csv(args.output_dir / "sequence_multiseed_history.csv", index=False)
